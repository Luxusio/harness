...
> - **Sub-file fallback.** The source loads `dx-hall-of-fame.md` as a sub-file. On Codex side, read it from `plugin/skills/plan-devex-review/dx-hall-of-fame.md` (Claude tree). DO NOT expect a Codex-native copy — v1.5 fallback architecture per spike-report §3.6. Methodology parity is preserved by reference.
...
For high-stakes DX ambiguity — public API or CLI design, breaking changes, deprecation timeline, destructive scope (removing a flag, renaming an SDK method, changing default behavior), missing context (no migration path, no compat strategy) — STOP. Name it in one sentence, present 2-3 options with concrete tradeoffs (developer impact, deprecation cost, migration effort), and ask via conversational prose:

```
<One-sentence description of the ambiguity.>
A) <Option with developer impact, deprecation cost, migration effort>
B) <Option with developer impact, deprecation cost, migration effort>
C) <Option with developer impact, deprecation cost, migration effort>
...
During each review pass, load the relevant section from `plugin/skills/plan-devex-review/dx-hall-of-fame.md` (Claude tree — Codex v1.5 sub-file fallback per spike-report §3.6).
...
If detected: State your classification and ask for confirmation via conversational prose. Do not ask from
scratch. "I'm reading this as a CLI Tool plan. Correct? Reply yes or correct me."
...
Then present concrete persona archetypes based on the detected product type via conversational prose:
...
Reply A / B / C / D, or describe your target developer.
```
...
Then SHOW it to the user via conversational prose:
...
Reply A / B / C, or describe what's wrong.
```
...
Then ask via conversational prose:
...
Load the relevant section from `plugin/skills/plan-devex-review/dx-hall-of-fame.md` (Claude tree) for gold standard examples.

Identify the most likely magical moment for this product type, then present delivery
vehicle options with tradeoffs via conversational prose:
...
Present three options via conversational prose:
...
3. **Conversational ask per friction point.** One question per friction point found.
...
4. Load Hall of Fame reference for this pass (read relevant section from `plugin/skills/plan-devex-review/dx-hall-of-fame.md` — Claude tree)
5. Fix: Edit the plan to add what's missing
6. Re-rate: "Now 7/10, still missing [specific gap]"
7. Ask via conversational prose if there's a genuine DX choice to resolve
...
- **DX EXPANSION:** After fixing to 10, also ask "What would make this dimension
  ...
  individual opt-in conversational asks.
...
**Anti-shortcut clause:** PLAN.md is the OUTPUT of the interactive review, not a substitute for it. Writing every finding into one plan write and signaling completion without firing a conversational ask is the precise failure mode the May 2026 transcript bug surfaced — the model explored, found issues, and dumped them into a deliverable rather than walking the user through them. If you have ANY non-trivial finding in any review pass (1-8), the path from finding to PLAN.md write goes THROUGH a conversational ask (see Conversational Ask Format above). Zero findings in every pass is the only path that bypasses asking. If you find yourself wanting to write a plan with findings before asking, stop — that's the bug.
...
```bash
git log --oneline --all -- 'doc/harness/**' 2>/dev/null | grep -i "dx\|devex\|developer" | head -5
```
...
**Load reference:** Read the `## Pass 1` section from `plugin/skills/plan-devex-review/dx-hall-of-fame.md` (Claude tree) before scoring this pass. Do NOT load the entire file.
...
**STOP.** Ask once per issue via conversational prose. Recommend + WHY. Reference the persona. One issue per ask. Do NOT batch.

### Pass 2: API/CLI/SDK Design (Usable + Useful)

**Load reference:** Read the `## Pass 2` section from `plugin/skills/plan-devex-review/dx-hall-of-fame.md` (Claude tree) before scoring this pass. Do NOT load the entire file.
...
**STOP.** Ask once per issue via conversational prose. Recommend + WHY. One issue per ask.

### Pass 3: Error Messages & Debugging (Fight Uncertainty)

**Load reference:** Read the `## Pass 3` section from `plugin/skills/plan-devex-review/dx-hall-of-fame.md` (Claude tree) before scoring this pass. Do NOT load the entire file.
...
**Load reference:** Read the `## Pass 4` section from `plugin/skills/plan-devex-review/dx-hall-of-fame.md` (Claude tree) before scoring this pass. Do NOT load the entire file.
...
**Load reference:** Read the `## Pass 5` section from `plugin/skills/plan-devex-review/dx-hall-of-fame.md` (Claude tree) before scoring this pass. Do NOT load the entire file.
...
**Load reference:** Read the `## Pass 6` section from `plugin/skills/plan-devex-review/dx-hall-of-fame.md` (Claude tree) before scoring this pass. Do NOT load the entire file.
...
**Load reference:** Read the `## Pass 7` section from `plugin/skills/plan-devex-review/dx-hall-of-fame.md` (Claude tree) before scoring this pass. Do NOT load the entire file.
...
**Load reference:** Read the `## Pass 8` section from `plugin/skills/plan-devex-review/dx-hall-of-fame.md` (Claude tree) before scoring this pass. Do NOT load the entire file.
...
`plugin/skills/plan-devex-review/dx-hall-of-fame.md` (Claude tree).

Check each item. For any unchecked item, explain what's missing and suggest the fix.

**STOP.** Ask via conversational prose for any item that requires a design decision.
...
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
mkdir -p doc/harness 2>/dev/null || true
echo '{"ts":"'"$_TS"'","type":"outside-voice","skill":"plan-devex-review","branch":"'"$_BRANCH"'","status":"STATUS","source":"SOURCE"}' >> doc/harness/learnings.jsonl 2>/dev/null || true
```
...
Follow the Conversational Ask Format described in the Preamble above. Additional rules for
DX reviews:

* **One issue = one conversational ask.** Never combine multiple issues.
...
Use the template from the **Empathy Narrative** section in `plugin/skills/plan-devex-review/dx-hall-of-fame.md` (Claude tree). Mandatory output — not optional.
...
Produce the full 9-stage table from the **Developer Journey Map** section in `plugin/skills/plan-devex-review/dx-hall-of-fame.md` (Claude tree).
...
conversational ask. Never batch. For DX debt: missing error messages, unspecified upgrade
...
Produce the 8-dimension scorecard from the **DX Scorecard** section in `plugin/skills/plan-devex-review/dx-hall-of-fame.md` (Claude tree).
Each dimension: score 0-10, finding, recommendation.

Also include the TTHW row from the **TTHW Assessment** section in that same file.
...
If any conversational ask goes unanswered, note here. Never silently default.
