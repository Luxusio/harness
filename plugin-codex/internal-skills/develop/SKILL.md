...
Then always spawn exactly one fresh `code_review_<unique>` with `fork_turns:"none"`. Compactly reserialize available results and escape literal `<`, `>`, `&` as `\u003c`, `\u003e`, `\u0026`. Include depth and concise evidence as narrative, not a schema field. Only this full-sweep formal reviewer owns `review-code`; hunters never emit verdicts or receipts. If its required narrative depth assessment is `ESCALATE_DEEP`, do not advance even on a mechanically valid PASS: increase depth and rerun necessary discovery plus a fresh formal reviewer.
