...
Fan-out is exact: LIGHT starts zero hunters; STANDARD starts exactly the selected hunter; DEEP starts both hunters in one batch. Every selected hunter is read-only, uses `fork_turns:"none"`, and reads `${HARNESS_PLUGIN_ROOT}/agents/defect-hunter.md` with only its assigned focus. When routed, start `security_review_<unique>` with discovery (or alongside formal review for LIGHT); security receives no hunter data.
