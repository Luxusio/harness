...
Fan-out is exact: LIGHT starts zero hunters; STANDARD starts exactly the selected hunter; DEEP starts both hunters in one batch. Every selected hunter is read-only, uses `fork_turns:"none"`, and reads `${HARNESS_PLUGIN_ROOT}/agents/defect-hunter.md` with only its assigned focus: `spawn_agent(task_name="defect_hunter_correctness_<unique>", fork_turns:"none")`; `spawn_agent(task_name="defect_hunter_contract_tests_<unique>", fork_turns:"none")`. When routed, start `security_review_<unique>` with discovery (or alongside formal review for LIGHT); security receives no hunter data.
...
rerun focused tests, recompute depth with the prior live depth as a floor, rerun only fresh discovery selected by the recomputed depth, then every routed formal lens including fresh `review-security`; security remains independent with no hunter payload and code review remains one fresh formal reviewer. Inline self-review is no fallback.
