...
- Every repo-mutating task follows: plan -> critic-plan PASS -> implement -> runtime QA -> critic-intent -> close. Documentation (DOC_SYNC + distilled change doc) is generated during develop phase.
- The only hard gate is at task completion: runtime critic verdict must PASS. Stale PASS (after file changes) does not count.
- DOC_SYNC.md is mandatory for all repo-mutating tasks (generated by develop phase).
...
- Protected artifacts have role ownership: PLAN.md=plan-skill, HANDOFF.md=developer, DOC_SYNC.md=developer, CRITIC__runtime.md=qa agents. Enforced by prewrite gate. For team tasks with an explicit synthesis owner, final runtime verification artifacts and final HANDOFF refresh are reserved to that synthesis owner once TEAM_SYNTHESIS.md is ready.
