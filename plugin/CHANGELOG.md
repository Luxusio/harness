...
- Task-owned source paths now survive clean commits from a validated, no-follow task baseline through review and verified install; new Git-backed tasks require baseline capture before state creation, invalid or unavailable baselines fail closed, and unchanged pre-task dirt remains excluded after commit. Closing a task synchronizes its active Goal child, Goal completion requires fresh receipt-backed QA for every closed child, and explicitly restarting a terminal Goal reactivates it without discarding its queue. All registered Claude hooks are fail-safe, and Codex QA prompts use the exact stable lifecycle-bound task names.
