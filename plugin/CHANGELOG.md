...
- Task-owned source paths now survive clean commits from a validated, no-follow task baseline through review and verified install; new Git-backed tasks require baseline capture before state creation and carry a protected generation marker, `rm`/`unlink` inspect every literal and bounded glob/brace deletion target, missing or unavailable required baselines fail closed, genuine legacy tasks remain compatible, and unchanged pre-task dirt stays excluded after commit. Closing a task synchronizes its active Goal child, Goal completion requires fresh receipt-backed QA for every closed child, and explicitly restarting a terminal Goal reactivates it without discarding its queue. All registered Claude hooks are fail-safe, and Codex QA prompts use the exact stable lifecycle-bound task names.
