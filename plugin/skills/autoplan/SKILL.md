...
## Filesystem Boundary — External AI Prompts

All prompts sent to external AI tools MUST be prefixed with this boundary instruction:
...
This prevents external AI tools from discovering skill files on disk and following their instructions instead of reviewing the plan.
...
echo "RESTORE_PATH=$HOME/.harness-plans/${BRANCH}-autoplan-restore-${DATETIME}.md"
```
...
- Dual voices: always run a Claude subagent (Agent tool) if available (P6).
  ...
  **Codex CEO voice** (via Bash):
  ...
  # [outside voice via Agent tool] -C "$_REPO_ROOT" -s read-only --enable web_search_cached
  ```
  Timeout: 10 minutes
