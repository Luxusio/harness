...
3. Emit/replace the Harness routing block in CLAUDE.md; touch `"$_MARKER_DIR/routing-injected"`.
...
below. Do not write the manifest yet on a fresh setup. Keep results in
`_PENDING_HEALTH_COMPONENTS`; apply them only after Phase 3 creates the
canonical manifest. On repair/upgrade, preserve an existing non-empty
...
enabled rather than asking a policy question. Apply the staged value after
Phase 3; never create a partial manifest during Phase 2.5.
...
Always allowed in plan mode: detection bash (read-only), writes to `doc/harness/`
(local.yaml, .markers/), AskUserQuestion.

**PLAN MODE EXCEPTION — always run:** marker file creation.
