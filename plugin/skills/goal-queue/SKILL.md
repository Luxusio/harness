...
Ask questions until the following are explicit. Bundle related questions in
`AskUserQuestion` when possible. For free-form product vision, plain prose is
acceptable, but end with concrete options where tradeoffs matter.
...
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/goal_queue_runner.py loop \
  ...
  --command-template 'claude -p "{prompt}"'
```
