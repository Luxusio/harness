...
## Compiled memory index rules

The compiled memory index (`harness/memory-index/`) is a **generated artifact**, not a source of truth.

### Generation rules
- The index is generated by `harness/scripts/build-memory-index.py` — a deterministic script
- It compiles records from durable docs, state, and policies into a structured JSON index
- **Never edit index files manually** — always regenerate with `bash harness/scripts/build-memory-index.sh`

### Rebuild triggers
After any modification to durable source files, rebuild the index:
1. `bash harness/scripts/build-memory-index.sh`
2. `bash harness/scripts/check-memory-index.sh`
...
### Temporal relations
- Superseded facts: do NOT delete — mark with `supersedes` relation in the source document
- Resolved unknowns: prefer resolution link over simple deletion
- Conflicts: flag rather than silently resolve

### Index vs source of truth
- Durable docs/state/policies remain the source of truth
- The index improves retrieval speed and accuracy
- Merge conflicts: resolve in source docs first, then regenerate index
