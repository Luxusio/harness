...
Use that JSON as the source of truth for routing and workflow state.
...
- run `hctl start`, `hctl context`, `hctl update`, `hctl verify`, `hctl close`
...
If `hctl close` blocks, fix the blocking condition instead of explaining it away.
