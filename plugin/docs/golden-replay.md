...
The packaged corpus no longer depends on live runtime task state under
`doc/harness/tasks/`. Historical task snapshots are rebuilt from a versioned
fixture bundle in `plugin/scripts/replay_fixtures.py` and materialized into a
hidden `.harness-replay-fixtures/` overlay during replay. That keeps runtime
state ephemeral while making the replay corpus deterministic in fresh clones,
zip exports, and CI.
...
## Packaged fixture policy

The default corpus sets `task_fixture_policy: packaged-first` and targets
logical task paths like `doc/harness/tasks/TASK__...`.

Replay resolves them in this order:

1. packaged replay fixture overlay generated by `plugin/scripts/replay_fixtures.py`
2. live task directory for corpora that do **not** opt into packaged-first mode

When the packaged overlay is used, generated paths are normalized back to the
logical historical task path before comparison. This preserves the historical
expectations in the corpus while removing the previous hidden dependency on
