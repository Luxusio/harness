...
def preview_handoff(task_dir, trigger=None, compaction_just_occurred=False):
    """Build the structured handoff payload without writing SESSION_HANDOFF.json.

    When ``trigger`` is omitted, this reuses should_create_handoff() so callers
    can preview the same recovery surface that session-end sync uses.
    Returns the handoff dict, or None when no trigger applies / the task is invalid.
    """
    state_file = os.path.join(task_dir, "TASK_STATE.yaml")
    if not os.path.isfile(state_file):
        return None

    resolved_trigger = trigger or should_create_handoff(
        task_dir, compaction_just_occurred=compaction_just_occurred
    )
    ...
    if team_recovery:
        pending_paths = [
            path for path in (team_recovery.get("pending_owned_paths") or [])
    ...
    next_step = _derive_next_step(resolved_trigger, task_dir, team_recovery=team_recovery)
...
def generate_handoff(task_dir, trigger):
    ...
    """
    handoff = preview_handoff(task_dir, trigger=trigger)
    if not handoff:
        return None
...
def _build_team_recovery(task_dir):
    ...
    incomplete_workers = [
        worker for worker in error_workers
        if worker and worker not in missing_workers
    ]
    pending_workers = []
    for worker in expected_workers:
        if worker in missing_workers or worker in incomplete_workers:
            pending_workers.append(worker)
    ready_workers = [worker for worker in expected_workers if worker not in pending_workers]
    has_worker_progress = bool(team_state.get("worker_summary_present_count") or team_state.get("worker_summary_ready"))
    synthesis_started = bool(team_state.get("synthesis_exists") or team_state.get("synthesis_ready"))

    phase = team_state.get("derived_status") or team_state.get("current_status") or "n/a"
    if not team_state.get("plan_ready"):
        phase = "plan"
    elif not has_worker_progress and not synthesis_started and bootstrap_state.get("available") and (
        not bootstrap_state.get("generated") or bootstrap_state.get("refresh_needed")
    ):
        phase = "bootstrap"
    elif not has_worker_progress and not synthesis_started and dispatch_state.get("available") and (
        not dispatch_state.get("generated") or dispatch_state.get("refresh_needed")
    ):
        phase = "dispatch"
    elif not has_worker_progress and not synthesis_started and launch_state.get("available") and (
        not launch_state.get("generated") or launch_state.get("refresh_needed")
    ):
        phase = "launch"
    ...
    if phase == "bootstrap" and bootstrap_state.get("available") and (
        not bootstrap_state.get("generated") or bootstrap_state.get("refresh_needed")
    ):
        bootstrap_index = str(bootstrap_state.get("bootstrap_index") or "team/bootstrap/index.json")
        if bootstrap_index not in pending_artifacts:
            pending_artifacts.append(bootstrap_index)
    if phase == "dispatch" and dispatch_state.get("available") and (
        not dispatch_state.get("generated") or dispatch_state.get("refresh_needed")
    ):
        dispatch_index = str(dispatch_state.get("dispatch_index") or "team/bootstrap/provider/dispatch.json")
        if dispatch_index not in pending_artifacts:
            pending_artifacts.append(dispatch_index)
    if phase == "launch" and launch_state.get("available") and (
        not launch_state.get("generated") or launch_state.get("refresh_needed")
    ):
        launch_manifest = str(launch_state.get("launch_manifest") or "team/bootstrap/provider/launch.json")
    ...
    if not team_state.get("synthesis_ready"):
        ...
        if team_state.get("team_doc_sync_needed") and doc_sync_artifact not in pending_artifacts:
        ...
        if team_state.get("team_document_critic_needed") and document_artifact not in pending_artifacts:
    ...
    for worker in expected_workers:
        ...
        worker_details[worker] = {
            "status": str(parsed.get("status") or ("missing" if worker in missing_workers else "ready")),
            "pending": bool(worker in pending_workers),
            "artifact": relpath,
            "owned_writable_paths": list(parsed.get("planned_owned_paths") or (team_state.get("plan_owned_paths") or {}).get(worker) or []),
    ...
    return {
        "status": team_state.get("derived_status") or team_state.get("current_status") or "n/a",
        "phase": phase,
        ...
        "verification_reason": str(team_state.get("team_runtime_verification_reason") or ""),
        "verification_owners": list(team_state.get("team_runtime_verification_owners") or []),
        ...
        "documentation_owner_label": str(team_state.get("team_documentation_owner_label") or ""),
        ...
        "document_critic_stale_after_docs": bool(team_state.get("team_document_critic_stale_after_docs")),
        ...
        "document_critic_owner_label": str(team_state.get("team_document_critic_owner_label") or ""),
        ...
        "synthesis_semantic_errors": list(team_state.get("synthesis_semantic_errors") or []),
        ...
        "launch_implement_dispatcher": str(launch_state.get("implement_dispatcher") or ""),
        ...
        "launch_execute_command_preview": str(launch_state.get("execute_command_preview") or ""),
        "launch_execute_fallback_available": bool(launch_state.get("execute_fallback_available")),
        "launch_execute_resolution_reason": str(launch_state.get("execute_resolution_reason") or ""),
...
def _derive_next_step(trigger, task_dir, team_recovery=None):
    ...
    if isinstance(team_recovery, dict):
        ...
        if phase == "bootstrap" and team_recovery.get("bootstrap_available") and (
            not team_recovery.get("bootstrap_generated") or team_recovery.get("bootstrap_refresh_needed")
        ):
            bootstrap_index = str(team_recovery.get("bootstrap_index") or "team/bootstrap/index.json")
            bootstrap_reason = str(team_recovery.get("bootstrap_reason") or "bootstrap is stale")
            ...
            return (
                f"Run team-bootstrap first to generate {bootstrap_index}, worker briefs, and env snippets before resuming fan-out."
            )
        if phase == "dispatch" and team_recovery.get("dispatch_available") and (
            not team_recovery.get("dispatch_generated") or team_recovery.get("dispatch_refresh_needed")
        ):
            dispatch_index = str(team_recovery.get("dispatch_index") or "team/bootstrap/provider/dispatch.json")
            dispatch_reason = str(team_recovery.get("dispatch_reason") or "dispatch is stale")
            ...
            return (
                f"Run team-dispatch after bootstrap to generate {dispatch_index}, provider launch prompts, and worker run helpers before resuming fan-out."
            )
        if phase == "launch" and team_recovery.get("launch_available") and (
            not team_recovery.get("launch_generated") or team_recovery.get("launch_refresh_needed")
        ):
            launch_manifest = str(team_recovery.get("launch_manifest") or "team/bootstrap/provider/launch.json")
            launch_reason = str(team_recovery.get("launch_reason") or "launch plan is stale")
            provider_prompt = str(team_recovery.get("launch_provider_prompt") or "")
            execute_fallback = bool(team_recovery.get("launch_execute_fallback_available"))
            ...
            return (
                f"Run team-launch after dispatch to generate {launch_manifest}, auto-refresh stale bootstrap / dispatch artifacts if needed, and prepare the default fan-out command before resuming worker execution."
            )
        synthesis_workers = [str(x).strip() for x in (team_recovery.get("synthesis_workers") or []) if str(x).strip()]
        ...
        relaunch_selection_reason = str(team_recovery.get("relaunch_selection_reason") or "")
        if phase == "plan":
            return (
        ...
        if phase == "verification":
            verification_reason = str(team_recovery.get("verification_reason") or "run final runtime verification after TEAM_SYNTHESIS.md")
            runtime_artifact = str(team_recovery.get("runtime_artifact") or "CRITIC__runtime.md")
            if relaunch_available and relaunch_worker and relaunch_phase == "final_runtime_verification":
                return (
        ...
        if phase == "documentation":
            documentation_reason = str(team_recovery.get("documentation_reason") or "refresh DOC_SYNC.md after final team runtime verification")
            doc_sync_artifact = str(team_recovery.get("doc_sync_artifact") or "DOC_SYNC.md")
            document_artifact = str(team_recovery.get("document_critic_artifact") or "CRITIC__document.md")
            doc_sync_owners = [str(x).strip() for x in (team_recovery.get("doc_sync_owners") or []) if str(x).strip()]
            document_critic_owners = [str(x).strip() for x in (team_recovery.get("document_critic_owners") or []) if str(x).strip()]
            doc_sync_preview = ", ".join(doc_sync_owners[:3]) or "writer"
            document_critic_preview = ", ".join(document_critic_owners[:3]) or "critic-document"
            if relaunch_available and relaunch_worker and relaunch_phase == "documentation_sync":
                return (
            ...
            if relaunch_available and relaunch_worker and relaunch_phase == "documentation_review":
                return (
        ...
        if phase == "complete" and handoff_refresh_needed:
            if relaunch_available and relaunch_worker and relaunch_phase == "handoff_refresh":
                return (
                    f"Use team-relaunch for {relaunch_worker}'s handoff refresh phase to update HANDOFF.md from the latest team artifacts before closing."
                )
            return (
                f"{handoff_refresh_reason or 'Refresh HANDOFF.md after the latest team artifact update'}, then resume close or recovery from that updated handoff."
            )
    ...
    steps = {
        ...
        "blocked_env_reentry": (
            f"Resolve the environment blocker ({blockers or 'see TASK_STATE.yaml'}) and use "
            "ENVIRONMENT_SNAPSHOT.md if present as the source of truth before resuming implementation."
        ),
...
def _build_read_first_list(task_dir, last_fail_evidence_refs, team_recovery=None):
    ...
    state_file = os.path.join(task_dir, "TASK_STATE.yaml")
    blocked_status = str(yaml_field("status", state_file) or "").strip().lower()
    was_blocked_env = str(yaml_field("was_blocked_env", state_file) or "").strip().lower() in ("true", "1", "yes")
    env_snapshot = "ENVIRONMENT_SNAPSHOT.md"
    if (blocked_status == "blocked_env" or was_blocked_env) and os.path.isfile(os.path.join(task_dir, env_snapshot)):
        candidates.append(env_snapshot)
    ...
    if isinstance(team_recovery, dict):
        ...
        if runtime_artifact and os.path.isfile(os.path.join(task_dir, runtime_artifact)):
        ...
        if doc_sync_artifact and os.path.isfile(os.path.join(task_dir, doc_sync_artifact)):
        ...
        if document_artifact and os.path.isfile(os.path.join(task_dir, document_artifact)):
            if document_artifact not in candidates:
                candidates.append(document_artifact)
        if team_recovery.get("handoff_refresh_needed") and os.path.isfile(os.path.join(task_dir, "HANDOFF.md")):
