#!/usr/bin/env python3
"""hygiene_restore.py — restore a doc archived by doc_hygiene.py.

Usage:
  python3 plugin/scripts/hygiene_restore.py <archive-path>

  <archive-path> must be a file under a _archive/ subdirectory,
  e.g. doc/changes/_archive/foo.md

Example:
  python3 plugin/scripts/hygiene_restore.py doc/changes/_archive/foo.md
...
def restore(archive_path: str, repo_root: str | None = None) -> int:
    ...
    source_dir = os.path.dirname(archive_dir)
    ...
    subprocess.run(
        ["git", "commit", "-m",
         f"hygiene: restore {rel_dest}\n\nRestored from archive via hygiene_restore.py."],
        cwd=repo_root, capture_output=True, text=True,
    )
...
def main() -> int:
    ...
    p = argparse.ArgumentParser(
        ...
        epilog=(
            "Example:\n"
            "  python3 plugin/scripts/hygiene_restore.py "
