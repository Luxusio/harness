...
- `setup_finalize.py` — applies canonical operational ignores, migrates legacy manifests to manifest `version: 6`, verifies setup resources and routing, and removes legacy `doc/harness/.version`/`.format-version` files; `SETUP_OK` (not the manifest version) is the success signal
