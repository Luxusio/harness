...
- `hygiene_scan.py` — SessionStart auto-hygiene: contract drift Tier A/B auto-apply + doc_hygiene invocation. State: `doc/harness/.hygiene-last-run`, `doc/harness/.hygiene-pending.json`, `doc/harness/.hygiene-observe.log` (legacy read fallback for old `.maintain-*` names; see `doc/harness/patterns/maintenance-state-naming.md`)
...
- `maintain_restore.py` — restore a file previously archived by doc_hygiene.py via `git mv`. Usage: `python3 plugin/scripts/maintain_restore.py <archive-path>` (legacy-compatible CLI name)
# Runtime services
