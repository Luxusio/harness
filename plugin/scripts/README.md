...
- `hygiene_scan.py` — SessionStart auto-hygiene: contract drift Tier A/B auto-apply + doc_hygiene invocation. State: `doc/harness/.maintain-last-run`, `doc/harness/.maintain-pending.json`, `doc/harness/.maintain-observe.log` (legacy-compatible names; see `doc/harness/patterns/maintenance-state-naming.md`)
