...
- `codex_lifecycle_watcher.py` — SessionStart and spawn-selective PreToolUse restore a safe root-rollout registration; MCP-hosted daemon threads accept only direct `collaboration.spawn_agent`, structured output, exactly one trusted depth-1 child rollout, and direct `FINAL_ANSWER` delivery, then record strictly correlated review/QA completions once. Activity events are ignored. The session marker and current `TASK.json` run identity are the sole task binding. Protocol drift leaves receipts missing and close fail-closed until Harness and Codex are upgraded together.
