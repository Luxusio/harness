...
# Reads project-specific rules from .claude-harness/arch-rules.yaml if present.
# arch-rules.yaml is a FALLBACK for repos without native lint boundary rules.
#
# Usage: scripts/agent/arch-check.sh

echo "=== repo-os architecture check ==="
echo ""
...
ARCH_RULES=""
if [[ -f ".claude-harness/arch-rules.yaml" ]]; then
  ARCH_RULES=".claude-harness/arch-rules.yaml"
