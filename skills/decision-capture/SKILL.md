...
- **constraint**: an ongoing rule (`docs/constraints/`)
- **decision**: a specific choice with rationale (`docs/decisions/` as ADR)
- **approval_rule**: a gate that requires user confirmation (`.claude-harness/policies/approvals.yaml`)
- **observed_fact**: verified technical finding (`docs/runbooks/` or `docs/architecture/`)
...
**For constraints:**
- Append to `docs/constraints/project-constraints.md`
- Keep the entry short: rule, scope, reason

**For decisions (ADR):**
- Create `docs/decisions/ADR-NNNN-<slug>.md` with:
  - Status: accepted
...
**For approval rules:**
- Add to `.claude-harness/policies/approvals.yaml` under `always_ask_before`

**For observed facts:**
- Add to the relevant `docs/runbooks/` or `docs/architecture/` file

### 4. Update recent decisions
Append a one-line entry to `.claude-harness/state/recent-decisions.md`:
```
