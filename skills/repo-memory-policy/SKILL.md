...
|------|----------|---------|
| `constraint` | `docs/constraints/` | "retry max 2 times" |
| `decision` | `docs/decisions/ADR-*.md` | "chose PostgreSQL over MongoDB" |
| `approval_rule` | `.claude-harness/policies/approvals.yaml` | "auth changes need confirmation" |
| `observed_fact` | `docs/runbooks/` or `docs/architecture/` | "service X calls Y synchronously" |
| `runbook_note` | `docs/runbooks/` | "restart worker after config change" |
| `hypothesis` | `.claude-harness/state/unknowns.md` | "billing module may have race condition" |
| `open_question` | `.claude-harness/state/unknowns.md` | "who owns the legacy auth middleware?" |
...
- **Action**: When exceeded, move older entries (beyond the most recent 50) to `.claude-harness/state/recent-decisions-archive.md` (append to archive, then remove from active file)
