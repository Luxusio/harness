...
- Read `docs/architecture/` if it exists
- Read `.claude-harness/manifest.yaml` for project structure hints
- Read `docs/constraints/project-constraints.md` for confirmed rules
- Check `.claude-harness/state/unknowns.md` for unresolved architecture questions
...
### 5. Record findings
- New confirmed boundary rules → `docs/architecture/` + `decision-capture`
- New hypotheses → `.claude-harness/state/unknowns.md`
...
3. `scripts/agent/arch-check.sh` addition
4. Documentation (last resort)

If a boundary rule is confirmed:
1. Append it to `.claude-harness/arch-rules.yaml` in the defined boundary format
2. Run `scripts/agent/arch-check.sh` to verify the rule catches existing violations
