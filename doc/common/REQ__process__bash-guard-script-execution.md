...
| A write whose relative path resolves **outside** the repo, including after a literal `cd` — `cd /tmp/work && echo hi > out.py`, `echo x > /tmp/RECEIPTS.jsonl` | allow | Outside the repo is outside the gate. All spellings of one command must agree; a basename alone does not make a path an artifact |
