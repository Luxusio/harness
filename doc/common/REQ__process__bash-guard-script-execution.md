...
| A **directory** operand covering an artifact — `rm -rf <taskdir>`, `mv <taskdir> /tmp/x`, `chmod -R 000 <taskdir>`, `git clean -fd doc/harness` | allow | Classification is by exact basename, so a directory is never a gated token. Unlike the rest of this table this is an *accident* shape rather than a determined-caller one; recorded as a known gap |
| A write whose relative path resolves outside the repo only **after a `cd`** — `cd /tmp/work && echo hi > out.py` | deny | Known over-block. The shell's cwd is not inferred: fifteen constructs run a `cd` that does not move the caller, and honouring any of them turns a real write to a protected artifact into an allow. Measured, not assumed — see the pattern doc and `test_cwd_is_not_inferred_from_cd` |
| A write naming an **absolute** path outside the repo — `echo x > /tmp/RECEIPTS.jsonl` | allow | Outside the repo is outside the gate; a basename alone does not make a path an artifact |
