...
| --- | --- | --- |
| REVIEW queue inspection from `.maintain-pending.json` | SessionStart/prompt memory, then normal task planning | Surface the item as context. If the current task touches that area, fold the decision into the task; otherwise leave it pending without forcing a separate maintain command. |
