...
> `stop_gate.py`; the Claude `Stop` hook registration was removed and
> `plugin/scripts/stop_gate.py` / `tests/test_stop_gate.py` were deleted the
> same day. The `stop_gate.py` instances below (module docstring claim,
> second instance) remain historical examples of the general rule this REQ
> states; they describe a defect found while the hook was still registered,
> not a claim about a script that still exists.
