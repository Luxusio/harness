...
| 12 | **Same envelope touches protected + unprotected files** | N/A — Claude's `Edit` is per-file, prewrite gate intercepts each | `apply_patch` envelope can mix protected (`PLAN.md`, `RECEIPTS.jsonl`) + unprotected files. `prewrite_gate.py` must scan ALL files in the envelope, not just the first | ⚠ caveat | The prewrite gate parses the full envelope and denies if any target is protected. |
