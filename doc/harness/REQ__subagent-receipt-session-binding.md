...
- The conflict marker carries no task authority. A fresh Codex PreToolUse event must place its exact `tool_use_id` into that recovery fence, and only the matching PostToolUse result may rebind; replaying either result that created the conflict remains non-authoritative.
