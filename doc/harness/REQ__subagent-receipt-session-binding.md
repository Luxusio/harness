...
- Only the bare Codex task tool ids and the exact Harness MCP-qualified ids are eligible for that handoff. A different MCP namespace that merely ends in `task_start` or `task_context` is not Harness authority.
- Exact-session publication is serialized. If a PostToolUse result conflicts with another still-open exact task/run for that session, Harness cannot prove which result is fresh and removes the existing binding. Work remains allowed, but neither task receives attestation until a later unambiguous exact result binds the session again.
- Conflict invalidation also removes the prior watcher registration. The later unambiguous bind checkpoints the rollout at its current offset, so events from the ambiguous interval cannot be replayed into the restored task.
- The conflict marker carries no task authority, only the two exact task/run generations that made selection ambiguous. While both remain open, repeated or fresh results cannot rebind either one. Once one generation becomes terminal or rotates, a valid result for the sole remaining open generation may establish a fresh binding.
- Recovery locks both conflict task directories in canonical path order and revalidates their controls before publication, so a concurrent resume cannot reopen the other generation between the decision and binding.
...
## Verification Cues
- `tests/test_codex_hook_wrappers.py` covers exact task-result binding, default-marker non-promotion, foreign-session isolation, conflicting cross-task authority invalidation, and fail-open PostToolUse behavior.
