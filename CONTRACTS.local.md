...
**Enforced by:** `plugin/skills/develop/SKILL.md` Phase 8.6 + `plugin/CLAUDE.md` trigger description. The document-review agent reads USER_FEEDBACK.jsonl, applies the "imperative + observable-surface + missing-from-current-REQs" heuristic, and reports candidate REQ gaps. The resolving task creates or updates the committed REQ directly or through `plugin/scripts/req_scaffold.py`; there is no MCP REQ writer. C-100 (bug -> REQ) covers the case where a user reported a defect explicitly; this contract covers the case where the user stated a requirement that the task closed without explicitly capturing.
